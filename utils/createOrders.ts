  ...
  try {
    ...
    `;
     ...
     const newOrders = [
       ...
       { order_id: 3, customer_id: 3, user_id: 3, status: 'Processing', total: 200.00, created_at: sevenMonthsAgo.toISOString() }
     ];
 
    for (const order of newOrders) {
      const orderExists = existingOrders.some(existingOrder => existingOrder.order_id === order.order_id);
      if (!orderExists) {
