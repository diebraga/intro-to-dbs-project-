...
export async function createDeliveries(): Promise<void> {
  ...
  try {
    const deliveryItems: DeliveryItem[] = [
      { delivery_id: 1, order_id: 1, delivery_date: '2023-02-01' },
      { delivery_id: 2, order_id: 2, delivery_date: '2023-02-02' },
      { delivery_id: 3, order_id: 3, delivery_date: '2023-02-03' }
    ];
    ...
    for (const item of deliveryItems) {
      ...
      if (count === 0) {
        ...
        const sqlInsert = `
          INSERT INTO deliveries (delivery_id, order_id, delivery_date, delivered_at, created_at)
          VALUES (?, ?, ?, NULL, CURRENT_TIMESTAMP)
        `;
        await db.run(sqlInsert, item.delivery_id, item.order_id, item.delivery_date);
